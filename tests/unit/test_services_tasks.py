...
def test_get_user_task_detail_not_found(dbsession, user):
    with pytest.raises(TaskNotFoundError):
        get_user_task_detail(dbsession, user_id=user.id, task_id=999)  # Assuming 999 doesn't exist
...
def test_get_user_task_detail_wrong_user(dbsession, user):
    ...
    other_user = User(first_name='Other', last_name='User', email='otheruser@example.com', password='test1234')
    dbsession.add(other_user)
    dbsession.commit()

    task = Task(owner_id=other_user.id, title='Other User Task', description='Another user\'s task')
    dbsession.add(task)
    dbsession.commit()
